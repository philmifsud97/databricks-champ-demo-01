...
resource "databricks_repo" "demo_in_user_home" {
  ...
  path       = "${data.databricks_current_user.me.repos}/demo01-tf-dev"
  branch     = "releases"
}
...
resource "databricks_repo" "demo_in_staging" {
  depends_on = [databricks_git_credential.global]
  url        = "https://github.com/${var.github_user_name}/${var.github_repo}.git"
  path       = "${data.databricks_current_user.me.repos}/demo01-tf-stg"
  branch     = "releases"
}
...
resource "databricks_repo" "demo_in_prod" {
  depends_on = [databricks_git_credential.global]
  url        = "https://github.com/${var.github_user_name}/${var.github_repo}.git"
  path       = "${data.databricks_current_user.me.repos}/demo01-tf-prod"
  branch     = "releases"
}
