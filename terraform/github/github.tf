...
resource "null_resource" "github_create_repo" {
  provisioner "local-exec" {
    command = "gh repo create ${var.github_repo} --template=https://github.com/philmifsud97/databricks-champ-demo-01 --public --include-all-branches"
    }
...
resource "databricks_token" "pat_for_devops" {
  comment          = "Github Demo01 TF (10 days)"
...
resource "github_actions_environment_variable" "cluserid_var_dev" {
...
resource "github_actions_environment_variable" "cluserid_var_stg" {
  repository       = data.github_repository.repo.name
